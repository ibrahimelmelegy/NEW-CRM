...
export function useActivityLog() {
    const sorted = computed(() =>
        [...activities.value].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
    );
    ...
    function getByContact(contactId: string | number): ActivityEntry[] {
        return sorted.value.filter(a =>
            ...
            (a.metadata as any)?.contactId === contactId
        );
    }
    ...
    const actionIcons: Record<string, string> = {
        created: 'ph:plus-circle', updated: 'ph:pencil', deleted: 'ph:trash',
        archived: 'ph:archive', converted: 'ph:arrows-clockwise', sent: 'ph:paper-plane-tilt',
        called: 'ph:phone', meeting: 'ph:calendar-check', note: 'ph:note-pencil', signed: 'ph:signature',
    };

    const actionColors: Record<string, string> = {
        created: '#22c55e', updated: '#3b82f6', deleted: '#ef4444',
        archived: '#f59e0b', converted: '#8b5cf6', sent: '#06b6d4',
        called: '#10b981', meeting: '#7c3aed', note: '#6b7280', signed: '#059669',
    };

    return {
        activities: sorted, recent,
        log, getByEntity, getByContact, clearAll,
        actionIcons, actionColors,
    };
}
