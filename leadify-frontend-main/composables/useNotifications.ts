...
export function useNotifications() {
    ...
    function markAsRead(id: string) {
        ...
        if (n) { n.read = true; save(notifications.value); }
    }
    ...
    function notifyDocumentCreated(refNumber: string, docType: string) {
        push({ type: 'success', category: 'document', title: 'Document Created', message: `${refNumber} (${docType}) created successfully.`, icon: 'ph:file-check' });
    }

    function notifyInvoiceOverdue(refNumber: string, daysOverdue: number) {
        push({ type: 'danger', category: 'invoice', title: 'Invoice Overdue', message: `${refNumber} is ${daysOverdue} days overdue.`, icon: 'ph:warning-circle', actionUrl: '/sales/invoices' });
    }
    ...
    return {
        notifications: sorted, unread, unreadCount,
        push, markAsRead, markAllAsRead, remove, clearAll,
        notifyDocumentCreated, notifyInvoiceOverdue, notifyReminder,
    };
}
