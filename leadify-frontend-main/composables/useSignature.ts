...
function load(): SignatureRecord[] { try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]'); } catch { return []; } }
function save(items: SignatureRecord[]) { localStorage.setItem(STORAGE_KEY, JSON.stringify(items)); }
...
export function useSignature() {
    ...
    function signDocument(id: string, signatureData: string) {
        ...
        if (rec) { rec.status = 'signed'; rec.signatureData = signatureData; rec.signedAt = new Date().toISOString(); save(signatures.value); }
    }

    function declineSignature(id: string) {
        const rec = signatures.value.find(s => s.id === id);
        if (rec) { rec.status = 'declined'; save(signatures.value); }
    }

    function removeSignature(id: string) { signatures.value = signatures.value.filter(s => s.id !== id); save(signatures.value); }
