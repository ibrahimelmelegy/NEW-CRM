...
export function useDocumentBuilder() {
  ...
  function getBuiltInTemplates(): ProTemplateDefinition[] {
    return [buildSalesProposal(), buildContractAgreement(), buildInvoiceTemplate(), buildMeetingNotes(), buildProjectBrief(), buildNDA()];
  }
  ...
  return {
    ...
    content: {
      ...
      content: [
        coverPage(
          'Sales Proposal',
          'Strategic partnership & customized solutions',
          'https://images.unsplash.com/photo-1557683316-973673baf926?q=80&w=2000&auto=format&fit=crop'
        ),
        heading(1, 'Executive Summary'),
        paragraph([text('Prepared for: '), variable('client.name')]),
        paragraph([text('Date: '), variable('invoice.date')]),
        rule(),
        ...
        paragraph([
          ...
          text(
            '. This document outlines our approach, deliverables, and pricing for the proposed engagement. Our solution is designed to address your specific requirements while delivering measurable business value.'
          )
        ]),
        ...
        paragraph([boldText('Total Investment: '), text('$34,000')]),
  ...
  return {
    ...
    content: {
      ...
      content: [
        coverPage(
          'Service Agreement',
          'Legally binding contract and terms',
          'https://images.unsplash.com/photo-1589829085413-56de8ae18c73?q=80&w=2000&auto=format&fit=crop'
        ),
        heading(1, 'Service Agreement'),
        paragraph([text('This Service Agreement ("Agreement") is entered into as of '), variable('invoice.date'), text(' by and between:')]),
        paragraph([
          boldText('Party A: '),
          variable('company.name'),
        ...
        paragraph([
          text(
            'This Agreement shall commence on the date first written above and shall continue for a period of twelve (12) months unless terminated earlier in accordance with Section 5 below.'
          )
        ]),
        heading(2, '3. Compensation'),
        paragraph([
          text(
            'The Client agrees to pay the Service Provider the total sum as outlined in the pricing schedule attached hereto as Exhibit A. Payment terms are Net 30 from the date of each invoice.'
          )
        ]),
        ...
        paragraph([
          text(
            'Either party may terminate this Agreement with thirty (30) days written notice. Upon termination, the Client shall pay for all services rendered up to the date of termination. The Service Provider shall deliver all completed work product to the Client.'
          )
        ]),
        heading(2, '6. Confidentiality'),
        paragraph([
          text(
            'Both parties agree to maintain the confidentiality of all proprietary information shared during the course of this engagement. This obligation shall survive the termination of this Agreement for a period of two (2) years.'
          )
        ]),
        heading(2, '7. Governing Law'),
        paragraph([
          text(
            'This Agreement shall be governed by and construed in accordance with the laws of the jurisdiction in which the Service Provider is located.'
          )
        ]),
        rule(),
...
function buildInvoiceTemplate(): ProTemplateDefinition {
  return {
    ...
    content: {
      ...
      content: [
        heading(1, 'INVOICE'),
        paragraph([variable('company.name')]),
        paragraph([variable('company.address')]),
        rule(),
        table(
          ['', ''],
          [
        ...
        heading(3, 'Bill To'),
        paragraph([variable('client.name')]),
        paragraph([variable('client.email')]),
        paragraph([variable('client.phone')]),
        rule(),
        ...
        paragraph([]),
        paragraph([italicText('Thank you for your business!')])
      ]
    }
  };
}
  ...
  return {
    ...
    content: {
      ...
      content: [
        ...
        paragraph([boldText('Date: '), variable('invoice.date')]),
        paragraph([boldText('Subject: '), text('Meeting with '), variable('client.name')]),
        paragraph([boldText('Location: '), text('Conference Room / Video Call')]),
        rule(),
        heading(2, 'Attendees'),
        bulletList(['Name / Role - Company', 'Name / Role - Company', 'Name / Role - Company']),
        heading(2, 'Agenda'),
        orderedList([
        ...
        bulletList(['Item 1 - Status: Completed / In Progress / Pending', 'Item 2 - Status: Completed / In Progress / Pending']),
        ...
        rule(),
        paragraph([italicText('Next meeting scheduled for: [Date and Time]')])
      ]
    }
  };
}
  ...
  return {
    ...
    content: {
      ...
      content: [
        ...
        paragraph([boldText('Prepared by: '), variable('company.name')]),
        paragraph([boldText('Client: '), variable('client.name')]),
        paragraph([boldText('Date: '), variable('invoice.date')]),
        rule(),
        heading(2, 'Project Overview'),
        paragraph([
          text(
            'Provide a high-level description of the project, its purpose, and the business problem it aims to solve. This section should give stakeholders a clear understanding of the project scope and vision.'
          )
        ]),
  ...
  return {
    ...
    content: {
      ...
      content: [
        ...
        paragraph([text('This Non-Disclosure Agreement ("Agreement") is entered into as of '), variable('invoice.date'), text(' by and between:')]),
        paragraph([boldText('Disclosing Party: '), variable('company.name'), text(', with its principal office at '), variable('company.address')]),
        paragraph([boldText('Receiving Party: '), variable('client.name')]),
        paragraph([text('Collectively referred to as the "Parties" and individually as a "Party".')]),
        rule(),
        heading(2, '1. Definitions'),
        paragraph([
          text(
            '"Confidential Information" means any and all non-public, proprietary, or confidential information disclosed by either Party to the other, whether orally, in writing, electronically, or by any other means, including but not limited to:'
          )
        ]),
        bulletList([
        ...
        paragraph([
          text(
            'This Agreement shall remain in effect for a period of two (2) years from the date of execution. The obligations of confidentiality shall survive the termination of this Agreement for an additional period of three (3) years.'
          )
        ]),
        heading(2, '5. Return of Information'),
        paragraph([
          text(
            'Upon written request or termination of this Agreement, the Receiving Party shall promptly return or destroy all copies of Confidential Information in its possession and certify in writing that it has done so.'
          )
        ]),
        ...
        paragraph([
          text(
            'This Agreement shall be governed by and construed in accordance with the laws of the jurisdiction in which the Disclosing Party maintains its principal place of business.'
          )
        ]),
