...
export default async function useTableFilter(position: string = 'lead', queryParams: Record<string, string | string[] | number[]> = {}) {
  ...
  try {
    ...
    router.replace({
      query: Object.entries(queryParamsFromUrl).reduce(
        (acc, [key, value]) => {
