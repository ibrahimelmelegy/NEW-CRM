...
export function useReminders() {
    ...
    function completeReminder(id: string) {
        ...
        if (r) { r.completed = true; r.completedAt = new Date().toISOString(); save(reminders.value); }
    }
    ...
    function updateReminder(id: string, updates: Partial<Reminder>) {
        const r = reminders.value.find(x => x.id === id);
        if (r) { Object.assign(r, updates); save(reminders.value); }
    }

    return {
        reminders, upcoming, overdue, today, completed, stats,
        addReminder, completeReminder, removeReminder, updateReminder,
    };
}
