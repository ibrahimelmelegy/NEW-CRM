...
onMounted(async () => {
  ...
  try {
    const response = await useApiFetch('auth/me') as Record<string, unknown>;
 ...
 */
function hasAccessToSection(navLink: { role?: string; submenu?: Array<{ role?: string }> }): boolean {
  ...
  if (navLink.submenu) {
    return navLink.submenu.some((sub: { role?: string }) => !getDisabled(sub.role));
  }
