...
el-table(:data="payslips" style="width: 100%" show-summary :summary-method="getSummary" border)
  el-table-column(label="Employee" min-width="180" fixed)
  ...
  //- Earnings columns
  el-table-column(label="Earnings" align="center")
    el-table-column(label="Basic" width="120" align="right")
      template(#default="{ row }")
        span(style="color: var(--text-primary)") {{ fmt(row.basicSalary) }}
    el-table-column(label="Housing" width="120" align="right")
      template(#default="{ row }")
        span(style="color: var(--text-primary)") {{ fmt(row.housingAllowance) }}
    el-table-column(label="Transport" width="120" align="right")
      template(#default="{ row }")
        span(style="color: var(--text-primary)") {{ fmt(row.transportAllowance) }}
    el-table-column(label="Other" width="110" align="right")
      template(#default="{ row }")
  ...
  //- Deductions columns
  el-table-column(label="Deductions" align="center")
    el-table-column(label="GOSI" width="120" align="right")
      template(#default="{ row }")
        span(style="color: #ef4444") {{ fmt(row.gosiDeduction) }}
    el-table-column(label="Absent" width="110" align="right")
      template(#default="{ row }")
        span(style="color: var(--text-primary)") {{ fmt(row.absentDeduction) }}
    el-table-column(label="Loan" width="110" align="right")
      template(#default="{ row }")
        span(style="color: var(--text-primary)") {{ fmt(row.loanDeduction) }}
    el-table-column(label="Other" width="110" align="right")
      template(#default="{ row }")
        span(style="color: var(--text-primary)") {{ fmt(row.otherDeductions) }}
    el-table-column(label="Total Ded." width="130" align="right")
      template(#default="{ row }")
        span.font-semibold(style="color: #ef4444") {{ fmt(row.totalDeductions) }}

  //- Net salary
  el-table-column(label="Net Salary" width="140" align="right" fixed="right")
    template(#default="{ row }")
