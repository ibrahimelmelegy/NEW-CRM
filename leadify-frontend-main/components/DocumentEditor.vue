<template>
  ...
  <div class="glass-panel shadow-md rounded-md py-2 px-6 flex flex-wrap gap-2 items-center" style="width: 100vw; position: fixed; z-index: 2">
    ...
    <button :class="buttonClass(isItalic)" @click="toggleItalic">
      <em>I</em>
    </button>
    <button :class="buttonClass(isUnderline)" @click="toggleUnderline">
      <u>U</u>
    </button>

    <!-- Add alignment buttons -->
    <div class="flex items-center border-l border-r border-gray-200 mx-2 px-2">
      <button :class="buttonClass(alignment === 'left')" class="p-1" @click="setAlignment('left')">
        <svg
      ...
      <button :class="buttonClass(alignment === 'center')" class="p-1" @click="setAlignment('center')">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
      ...
      <button :class="buttonClass(alignment === 'right')" class="p-1" @click="setAlignment('right')">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
      ...
      <button :class="buttonClass(alignment === 'justify')" class="p-1" @click="setAlignment('justify')">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
    ...
    >
      <option v-for="size in fontSizes" :key="size" :value="size" class="text-purple-700">{{ size }}px</option>
    </select>
    <input
      v-model="fontColor"
      type="color"
    ...
    <button class="relative group px-3 py-1 border rounded bg-white text-black hover:bg-gray-100" @click="triggerImageUpload">
    ...
    <button
      ...
      "
    >
      <img src="/images/pdf-icons/subtitle.svg" alt="Add Subtitle" class="w-5 h-5" />
    ...
    <button
      ...
      "
    >
      <img src="/images/pdf-icons/description.svg" alt="Add Description" class="w-5 h-5" />
  ...
  <div ref="editor" class="editor" @paste="handlePaste">
    <!-- Page overlays (headers, footers, page numbers, ...) -->
    <div v-if="overlay" ref="overlays" class="overlays">
      <div
    ...
    <div ref="content" class="content" :contenteditable="editable" :style="page_style(-1)" @input="input" @keyup="process_current_text_style">
...
export default {
  ...
  setup() {
    ...
    const setAlignment = align => {
      execCommand('justify' + (align === 'left' ? 'Left' : align === 'center' ? 'Center' : align === 'right' ? 'Right' : 'Full'));
    ...
    const buttonClass = active => `px-3 py-1 border rounded ${active ? 'bg-[#ccbcf8] text-white' : 'bg-white text-black hover:bg-gray-100'}`;
    return {
      fontSizes,
      fontSize,
      fontColor,
  ...
  methods: {
    handlePaste(event) {
      ...
      const isTable = pastedText.includes('\t') || /<table[^>]*>/.test(pastedText);
          ...
          if (cells.filter(el => el !== '').length === 1) {
            ...
            cells.forEach((cell, index) => {
              ...
              if (cells?.[index + 1]?.trim() == '') {
                colspan = colspan + 1;
                if (cells?.[index] && cells?.[index]?.trim() !== '') data = cells?.[index];
              }
              if (colspan > 1 && data) tableHTML += `<td colspan="${colspan}" style='padding: 12px; border: 1px solid #ddd;'>${data}</td>`;
    ...
    reset_content() {
      ...
      if (this?.invoiceData?.proposalDetails?.content == '' || this?.invoiceData?.proposalDetails?.content == null) {
        ...
        allPage?.forEach((el, index) => {
          el.querySelectorAll('#titleEdit').forEach(el1 => {
            if (el1.innerHTML.trim() === '2.Finance Table :') {
                  ...
                  this?.financeTable?.financeTable?.items?.map((el, index) => {
                    ...
                    const baseObj = {
                      ...
                      totalPrice:
                        ...
                        (el?.unitPrice -
                          el?.marginAmount +
                          ((el?.unitPrice - el?.marginAmount) * this?.invoiceData?.proposalDetails?.financeTable?.marginPercentage) / 100),
                      unitPrice:
                        el?.unitPrice -
                        el?.marginAmount +
                        ((el?.unitPrice - el?.marginAmount) * this?.invoiceData?.proposalDetails?.financeTable?.marginPercentage) / 100,
              ...
              tableStatic.columns = tableStatic.columns?.sort((a, b) => a.value - b.value);
              tableHTML = `
    ...
    ">
      ...
      <tbody>`;
              ...
              const totalPrice = Number(
                this?.invoiceData?.proposalDetails?.financeTable?.items.reduce((sum, item) => {
                  return (
                    sum +
                    item?.qty *
                      (item?.unitPrice -
                        item?.marginAmount +
                        ((item?.unitPrice - item?.marginAmount) * this?.invoiceData?.proposalDetails?.financeTable?.marginPercentage) / 100)
                  );
                }, 0)
              );
              const vat =
                (this?.invoiceData?.proposalDetails?.financeTable?.items.reduce((sum, item) => {
                  ...
                  (this?.invoiceData?.proposalDetails?.financeTable?.discount || 0)) *
                  0.15 || 0;
              const discount = Number(this?.invoiceData?.proposalDetails?.financeTable?.discount) || 0;
              const marginPercentage = Number(this?.invoiceData?.proposalDetails?.financeTable?.marginPercentage) || 0;
              ...
              const finalTotal =
                ...
                ) - Number(this?.invoiceData?.proposalDetails?.financeTable?.discount) || 0;
              tableHTML += `
