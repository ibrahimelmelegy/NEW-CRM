...
async function exportToPDF() {
  ...
  try {
    ...
    const checkNewPage = (doc: unknown, y: number, minSpace: number = 40) => {
    ...
    doc.setTextColor(50, 31, 107);
    doc.text('Total Material Cost', 12, (doc as unknown).lastAutoTable.finalY + 20);
    ...
    doc.setFont('helvetica', 'bold'); // Gilroy is not a default font in jsPDF
    doc.setTextColor(50, 31, 107);
    doc.text('Total Manpower Cost', 12, (doc as unknown).lastAutoTable.finalY + 20);
    ...
    doc.setFont('helvetica', 'bold'); // Gilroy is not a default font in jsPDF
    doc.setTextColor(50, 31, 107);
    doc.text('Total Assets Cost', 12, (doc as unknown).lastAutoTable.finalY + 20);
    ...
    doc.setFont('helvetica', 'bold'); // Gilroy is not a default font in jsPDF
    doc.setTextColor(50, 31, 107);
    doc.text('Total Costs', 12, (doc as unknown).lastAutoTable.finalY + 20);
    const summaryCards = [
      {
    ...
    const startX = 12;
    let x = startX;
    let yCards = (doc as unknown).lastAutoTable.finalY + 25;
    const cardsPerRow = 2;
