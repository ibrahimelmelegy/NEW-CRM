  ...
  el-form.border-t.pt-4(  autocomplete="off"   @submit.prevent='onSubmit'   ref="myForm" label-position="top"  :validationSchema="formSchema" )
    ...
    template(v-if="isAdditionalMaterial")
      ...
      .flex.align-center.gap-1
        ...
        el-button(size='medium' :icon="Edit" native-type="button" @click='selectedMaterial=addMaterials?.find((item) => item.id === addMaterialId),addAdditionMaterial = true' v-if="addMaterialId" class="!rounded-2xl !border-[#e9e8eb] !color-[#e9e8eb] !py-7 !px-4 !m-0")
    ...
    template(v-if="isAdditionalService")
      ...
      .flex.align-center.gap-1
        InputSelect.flex-1(placeholder=" Service Type" name="serviceId" :options="services" :value="isNewlyAdded ? services[0]?.value : serviceId? services?.find((item) => item.value === serviceId)?.value :data?.serviceId" :key="isNewlyAdded" @change="toggleServiceSelection" )
        ...
        el-button(size='medium' :icon="Edit" native-type="button" @click='selectedService=services?.find((item) => item.value === serviceId),addService = true' v-if="serviceId" class="!rounded-2xl !border-[#e9e8eb] !color-[#e9e8eb] !py-7 !px-4 !m-0")
      InputText.mt-4(:label="$t('operations.servicePrice')" type="number" disabled name="servicePrice" :value="services.find((service) => service.value === values.serviceId)?.price || 0" :key="values.serviceId" )
    .dialog-footer
...
const serviceId = ref();
const addMaterialsItems = ref<Record<string, unknown>>();
const selectedService = ref<Service>();
const finalAddMaterialsItems = ref<Record<string, unknown>>([]);
const formSchema = yup.object({
  ...
  unitPrice: yup
    .string()
    .required()
    .test('is-valid-number', 'Please enter a valid number.', (value: unknown) => /^\d*\.?\d*$/.test(value || ''))
...
function toggleAddMaterialSelection(val?: unknown) {
  ...
  if (props.selectedAddMaterialItem && addMaterial.value && Object.keys(props.selectedAddMaterialItem).length) {
    ...
    getAddMaterialsItem({ [addMaterialId.value]: props.allAddMaterialItems[addMaterialId.value as unknown] });
  }
}
/**
...
function getAddMaterialsItem(val: unknown) {
...
function toggleServiceSelection(val?: unknown) {
