...
async function loadPipelineChart() {
  ...
  if (success && body) {
    ...
    const chartData = stages.map((s: Record<string, unknown>) => ({
    ...
    chartOption.value = {
      ...
      xAxis: {
        type: 'category',
        data: chartData.map((d: { name: string; value: number }) => d.name),
      ...
      series: [{
        type: 'bar',
        barWidth: '40%',
        data: chartData.map((d: { name: string; value: number }) => d.value),
        itemStyle: {
...
async function loadLeadSourcesChart() {
  ...
  if (success && body) {
    ...
    const chartData = sources.length
      ? sources.map((s: Record<string, unknown>) => ({ name: s.name || s.source, value: s.count || s.value || 0 }))
      : [
...
async function loadFunnelChart() {
  ...
  if (success && body) {
    ...
    const funnelData = stages.length
      ? stages.map((s: Record<string, unknown>) => ({ name: s.name || s.stage, value: s.count || s.value || 0 }))
      : [
    ...
    funnelData.sort((a: { name: string; value: number }, b: { name: string; value: number }) => b.value - a.value);
    chartOption.value = {
...
async function loadRevenueTrendChart() {
  ...
  if (success && body) {
    ...
    const chartData = items.length
      ? items.map((item: Record<string, unknown>) => ({ name: (item.label || item.month || item.period) as string, value: (item.value || item.revenue || 0) as number }))
      : [];
