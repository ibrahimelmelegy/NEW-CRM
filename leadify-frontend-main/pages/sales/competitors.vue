...
const mockCompetitors: Competitor[] = [
  {
    ...
    strengths: ['Brand Recognition', 'AI Features', 'Integrations'], weaknesses: ['High Pricing', 'Complex Setup'],
    swot: {
  ...
  },
  {
    ...
    strengths: ['Local Support', 'Arabic UI', 'Affordable'], weaknesses: ['Limited Features', 'Small Team'],
    swot: {
  ...
  },
  {
    ...
    strengths: ['Market Leader', 'Enterprise Ready', 'Global Presence'], weaknesses: ['No Arabic', 'Poor Local Support', 'Expensive'],
    swot: {
  ...
  },
  {
    ...
    strengths: ['Easy to Use', 'Free Tier'], weaknesses: ['Basic CRM', 'No Customization', 'Startup'],
    swot: {
...
async function loadData() {
  ...
  try {
    ...
    const { body: competitorsData, success: competitorsOk } = await useApiFetch('competitors' as any);
    if (competitorsOk && Array.isArray(competitorsData)) {
      competitors.value = competitorsData;
    } else {
      competitors.value = mockCompetitors;
    }

    // Fetch win/loss records from competitor activity
    try {
      const { body: activityData, success: activityOk } = await useApiFetch('competitors/activity' as any);
      if (activityOk && Array.isArray(activityData)) {
        winLossRecords.value = activityData;
      } else {
        winLossRecords.value = mockWinLossRecords;
      }
    } catch {
      winLossRecords.value = mockWinLossRecords;
    }

    // Fetch threat matrix / comparison features
    try {
      const { body: matrixData, success: matrixOk } = await useApiFetch('competitors/threat-matrix' as any);
      if (matrixOk && Array.isArray(matrixData)) {
        comparisonFeatures.value = matrixData;
      } else {
        comparisonFeatures.value = mockComparisonFeatures;
      }
    } catch {
      comparisonFeatures.value = mockComparisonFeatures;
    }
