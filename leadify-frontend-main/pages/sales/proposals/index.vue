...
function computeTotal(p: unknown): number {
...
const stats = computed(() => {
  ...
  const pipelineValue = all.reduce((sum, p) => {
...
// ─── Actions ─────────────────────────────────────────────────────────
async function handleSubmitForApproval(row: unknown) {
  try {
...
function openRejectDialog(row: unknown) {
...
async function handleArchive(row: unknown) {
  try {
...
async function handleDownloadPdf(row: unknown) {
