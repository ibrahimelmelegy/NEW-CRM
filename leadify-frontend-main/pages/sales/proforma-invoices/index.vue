...
.p-6.animate-entrance
  ModuleHeader(
    ...
    subtitle="Manage proforma invoices for pre-shipment billing"
  )
    template(#actions)
      NuxtLink(to="/sales/proforma-invoices/create")
        el-button(
          type="primary"
          size="large"
  ...
  .mt-8(v-else)
    el-card.rounded-2xl(shadow="never" style="border: 1px solid var(--border-default);")
      .flex.flex-col.items-center.justify-center.text-center.py-12
        Icon(name="ph:receipt-bold" size="64" class="text-purple-300 mb-4 opacity-50")
        h3.text-2xl.font-bold(style="color: var(--text-primary)") No Proforma Invoices
        p.text-muted.mt-2.max-w-md Create proforma invoices for pre-shipment billing using the Document Builder.
        NuxtLink(to="/sales/proforma-invoices/create").mt-6
          el-button(type="primary" size="large" class="!rounded-xl") Create Your First Proforma Invoice
</template>
...
archiveTypeFilter.value = 'proforma_invoice';
...
async function handleDelete(row: any) {
  try {
    await ElMessageBox.confirm(`Delete proforma invoice "${row.title}"?`, 'Delete', { type: 'warning' });
    permanentlyDelete(row.id, 'proforma_invoice');
    ElMessage.success('Proforma invoice removed');
  } catch { /* cancelled */ }
}
</script>
