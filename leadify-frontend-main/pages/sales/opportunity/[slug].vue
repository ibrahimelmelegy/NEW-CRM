  ...
  el-tab-pane(:label="$t('opportunities.proposal')" , name="proposal")
    ...
    el-tab-pane(:label="$t('leads.activity')", name="activity")
      ...
      .flex.justify-center.items-center.w-full
        el-button( v-if="(activity?.docs?.length ?? 0) > 0" :loading = "loading" class="!rounded-2xl mb-2"  type= 'primary' size="large" :disabled="activity?.pagination?.totalPages === activity?.pagination?.page" @click="getActivityPage(Number(activity?.pagination?.page)+1)") {{ $t('common.view') }} More
  el-tab-pane(:label="$t('common.timeline')" name="timeline")
    RecordTimeline(:entityType="'opportunity'" :entityId="route.params.slug as string")
  el-tab-pane(:label="$t('common.comments')" name="comments")
    RecordComments(:entityType="'opportunity'" :entityId="route.params.slug as string")
  el-tab-pane(:label="$t('common.attachments')" name="record-attachments")
    RecordAttachments(:entityType="'opportunity'" :entityId="route.params.slug as string")
</template>
