...
.p-6.animate-entrance
  ...
  el-card.rounded-2xl(shadow="never" style="border: 1px solid var(--border-default);" v-loading="loading")
    el-table(:data="companies" style="width: 100%" :empty-text="$t('companies.noCompanies')")
      el-table-column(:label="$t('companies.company')" min-width="250")
      ...
      el-table-column(:label="$t('companies.website')" width="200")
        template(#default="{ row }")
          a.text-xs.text-violet-600(v-if="row.website" :href="row.website" target="_blank") {{ row.website }}
          span.text-gray-400(v-else) —
      el-table-column(:label="$t('companies.contacts')" width="100" align="center")
        template(#default="{ row }")
          el-badge(:value="row.contactCount" type="primary")
      el-table-column(:label="$t('companies.revenue')" width="140" align="right")
        template(#default="{ row }")
          span.font-mono.font-bold.text-sm {{ row.annualRevenue?.toLocaleString() || '—' }}
      el-table-column(:label="$t('companies.status')" width="100")
        template(#default="{ row }")
  ...
  el-dialog(v-model="showDialog" :title="$t('companies.addCompany')" width="560px")
    el-form(label-position="top" size="large")
      el-form-item(:label="$t('companies.companyName')")
        el-input(v-model="form.name" placeholder="Acme Corp")
      .grid.grid-cols-2.gap-4
        el-form-item(:label="$t('companies.industry')")
        ...
        el-form-item(:label="$t('companies.status')")
          el-select(v-model="form.status" class="w-full")
            el-option(label="Active" value="active")
            el-option(label="Prospect" value="prospect")
            el-option(label="Inactive" value="inactive")
      .grid.grid-cols-2.gap-4
        el-form-item(:label="$t('companies.website')")
          el-input(v-model="form.website" placeholder="https://...")
        el-form-item(:label="$t('companies.annualRevenue')")
...
async function fetchCompanies() {
  loading.value = true;
  try {
    const { body, success } = await useApiFetch('client?type=company', 'GET');
    if (success && body) {
...
async function saveCompany() {
  saving.value = true;
  try {
    const payload = { ...form, type: 'company', contactCount: 0 };
