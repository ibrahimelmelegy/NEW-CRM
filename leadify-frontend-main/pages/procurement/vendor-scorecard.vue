...
function getOverallScore(row: unknown): number {
...
const stats = computed(() => {
  ...
  const avgQuality = total ? data.reduce((sum, s) => sum + (s.qualityScore || 0), 0) / total : 0;
  const avgDelivery = total ? data.reduce((sum, s) => sum + (s.deliveryScore || 0), 0) / total : 0;
  const avgOverall = total ? data.reduce((sum, s) => sum + getOverallScore(s), 0) / total : 0;
...
function openEditDialog(scorecard: unknown) {
...
async function handleDelete(scorecard: unknown) {
  try {
