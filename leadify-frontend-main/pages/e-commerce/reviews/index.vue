...
.p-6.animate-entrance
  .flex.items-center.justify-between.mb-8
    div
      h1.text-3xl.font-black.tracking-tight(style="color: var(--text-primary);") {{ $t('ecommerce.reviews.title') || 'Product Reviews' }}
      p.text-sm.mt-1(style="color: var(--text-muted);") {{ $t('ecommerce.reviews.subtitle') || 'Moderate customer reviews and respond to feedback' }}
    ...
    .p-5.rounded-2xl.border(style="border-color: var(--border-default); background: var(--bg-elevated);")
      .flex.items-center.gap-3
        ...
        div
          ...
          p.text-xs.font-bold.uppercase.tracking-widest(style="color: var(--text-muted);") {{ $t('ecommerce.reviews.totalReviews') || 'Total Reviews' }}
    .p-5.rounded-2xl.border(style="border-color: #f59e0b33; background: var(--bg-elevated);")
      .flex.items-center.gap-3
        ...
        div
          ...
          p.text-xs.font-bold.uppercase.tracking-widest(style="color: var(--text-muted);") {{ $t('ecommerce.reviews.pendingModeration') || 'Pending Moderation' }}
    .p-5.rounded-2xl.border(style="border-color: var(--border-default); background: var(--bg-elevated);")
      .flex.items-center.gap-3
        .w-10.h-10.rounded-xl.flex.items-center.justify-center(style="background: rgba(245, 158, 11, 0.15)")
        ...
        div
          ...
          p.text-xs.font-bold.uppercase.tracking-widest(style="color: var(--text-muted);") {{ $t('ecommerce.reviews.avgRating') || 'Average Rating' }}
    .p-5.rounded-2xl.border(style="border-color: var(--border-default); background: var(--bg-elevated);")
      .flex.items-center.gap-3
        ...
        div
          ...
          p.text-xs.font-bold.uppercase.tracking-widest(style="color: var(--text-muted);") {{ $t('ecommerce.reviews.approvedReviews') || 'Approved Reviews' }}

  //- Filter Bar
  .flex.flex-wrap.items-center.gap-3.mb-6
    el-input(
      v-model="searchQuery"
      :placeholder="$t('ecommerce.reviews.searchPlaceholder') || 'Search by title or comment...'"
      clearable
      class="!w-64"
      size="default"
    )
    ...
    el-radio-group(v-model="filterStatus" size="default")
      ...
      el-radio-button(value="PENDING")
        .flex.items-center.gap-1
          | {{ $t('ecommerce.reviews.pending') || 'Pending' }}
          el-badge(:value="pendingCount" :hidden="!pendingCount" type="warning" class="ml-1")
      el-radio-button(value="APPROVED") {{ $t('ecommerce.reviews.approved') || 'Approved' }}
      el-radio-button(value="REJECTED") {{ $t('ecommerce.reviews.rejected') || 'Rejected' }}

    el-select(v-model="filterRating" :placeholder="$t('ecommerce.reviews.filterByRating') || 'All Ratings'" clearable size="default")
      el-option(:label="$t('ecommerce.reviews.allRatings') || 'All Ratings'" value="")
    ...
    el-card.rounded-2xl(
          ...
          div
            .flex.items-center.gap-2
              ...
              el-tag(v-if="review.isVerifiedPurchase" type="success" size="small" round effect="plain")
                ...
                | {{ $t('ecommerce.reviews.verified') || 'Verified Purchase' }}
      ...
      .p-4.rounded-xl.mb-3(v-if="review.merchantResponse || review.response" style="background: var(--bg-elevated); border: 1px solid var(--border-default);")
        .flex.items-center.gap-2.mb-2
          ...
          span.text-xs.font-bold.uppercase.tracking-widest(style="color: var(--text-muted);") {{ $t('ecommerce.reviews.merchantResponse') || 'Merchant Response' }}
      ...
      .flex.flex-wrap.items-center.gap-2.pt-3(style="border-top: 1px solid var(--border-default);")
        el-button(
          ...
          | {{ $t('ecommerce.reviews.approve') || 'Approve' }}
        el-button(
          ...
          | {{ $t('ecommerce.reviews.reject') || 'Reject' }}
        el-button(
          size="small"
          plain
          ...
          | {{ $t('ecommerce.reviews.respond') || 'Respond' }}
        .flex-1
        el-button(
          size="small"
          text
  ...
  .rounded-2xl.border-2.border-dashed.text-center.p-12(v-if="!loading && filteredReviews.length === 0" style="border-color: var(--border-default); color: var(--text-muted);")
    ...
    p.text-sm.mt-3 {{ $t('ecommerce.reviews.noReviews') || 'No reviews found matching your filters.' }}
  ...
  //- Respond Dialog
  el-dialog(v-model="showRespondDialog" :title="$t('ecommerce.reviews.respondToReview') || 'Respond to Review'" width="560px")
    //- Review Preview
    ...
    el-form(label-position="top" size="large")
      el-form-item(:label="$t('ecommerce.reviews.yourResponse') || 'Your Response'" required)
        el-input(
          v-model="respondText"
          type="textarea"
          :rows="4"
          :placeholder="$t('ecommerce.reviews.responsePlaceholder') || 'Thank you for your feedback...'"
        )
    template(#footer)
      el-button(@click="showRespondDialog = false") {{ $t('common.cancel') || 'Cancel' }}
      el-button(type="primary" :loading="respondLoading" @click="submitResponse" style="border-radius: 12px;") {{ $t('ecommerce.reviews.submitResponse') || 'Submit Response' }}
</template>
...
async function submitResponse() {
  if (!respondText.value.trim()) {
    ElMessage.warning('Response text is required')
    return
  }
  ...
  try {
    ...
    if (res?.success !== false) {
      ...
      ElMessage.success('Response submitted')
    }
  } finally {
