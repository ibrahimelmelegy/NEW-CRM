          ...
          <div v-for="rule in assignmentRules" :key="rule.id" class="glass-panel p-5 rounded-xl hover:border-primary-500/30 transition-all">
            ...
            <div class="mt-3 flex items-center justify-between">
              <div class="flex items-center gap-2 text-xs text-slate-400">
                ...
                <span>Assign to: <strong class="text-slate-200">{{ rule.assignTo }}</strong></span>
        ...
        <div class="space-y-4">
          <div v-for="esc in escalationRules" :key="esc.id" class="glass-panel p-5 rounded-xl hover:border-primary-500/30 transition-all">
            ...
            <div class="mt-4 space-y-2">
              <div v-for="(level, idx) in esc.levels" :key="idx"
                class="flex items-center gap-3 p-2 rounded-lg"
                :class="idx === 0 ? 'bg-amber-500/5' : idx === 1 ? 'bg-orange-500/5' : 'bg-red-500/5'"
              >
                <div class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold"
                  :class="idx === 0 ? 'bg-amber-500/20 text-amber-400' : idx === 1 ? 'bg-orange-500/20 text-orange-400' : 'bg-red-500/20 text-red-400'"
                >
                  L{{ idx + 1 }}
                </div>
                <div class="flex-1">
            ...
            <div class="mt-3 flex items-center gap-2 text-xs text-slate-500">
              <span>Entity: <strong class="text-slate-300">{{ esc.entity }}</strong></span>
        ...
        <div class="space-y-4">
          <div v-for="pool in roundRobinPools" :key="pool.id" class="glass-panel p-5 rounded-xl">
            ...
            <div class="flex items-center gap-4 text-xs text-slate-500">
              ...
              <span>Next: <strong class="text-slate-300">{{ pool.nextAssignee }}</strong></span>
        ...
        <el-form-item label="Conditions">
          <div class="space-y-2 w-full">
            ...
            <el-button text type="primary" @click="newRule.conditions.push({ field: 'source', operator: 'equals', value: '' })">
              <Icon name="ph:plus-bold" class="w-4 h-4 mr-1" /> Add Condition
            </el-button>
          </div>
        </el-form-item>
        <div class="grid grid-cols-2 gap-4">
...
const newRule = ref({
  name: '', type: 'ASSIGNMENT', entity: 'LEAD', matchType: 'ALL',
  conditions: [{ field: 'source', operator: 'equals', value: '' }],
  assignTo: '', method: 'ROUND_ROBIN'
});

const rules = ref([
  { id: 1, name: 'High-value leads → Senior Sales', description: 'Auto-assign leads with value > 50K to senior team', isActive: true, type: 'ASSIGNMENT',
    matchType: 'ALL', conditions: [{ field: 'Value', operator: '>', value: '50,000 SAR' }, { field: 'Source', operator: 'equals', value: 'Website' }],
    assignTo: 'Senior Sales Team', assignMethod: 'Round Robin', triggerCount: 87 },
  { id: 2, name: 'Enterprise leads → Key Account Mgr', description: 'Route enterprise segment leads to KAMs', isActive: true, type: 'ASSIGNMENT',
    matchType: 'ANY', conditions: [{ field: 'Industry', operator: 'equals', value: 'Enterprise' }, { field: 'Employees', operator: '>', value: '500' }],
    assignTo: 'Khalid Ibrahim', assignMethod: 'Direct', triggerCount: 34 },
  { id: 3, name: 'Support tickets by category', description: 'Auto-route tickets based on category', isActive: true, type: 'ASSIGNMENT',
    matchType: 'ALL', conditions: [{ field: 'Category', operator: 'equals', value: 'Technical' }],
    assignTo: 'Tech Support Pool', assignMethod: 'Least Loaded', triggerCount: 156 },
  { id: 4, name: 'Regional lead routing', description: 'Route leads by geographic region', isActive: false, type: 'ASSIGNMENT',
    matchType: 'ALL', conditions: [{ field: 'Region', operator: 'equals', value: 'GCC' }],
    assignTo: 'GCC Sales Team', assignMethod: 'Weighted', triggerCount: 12 }
]);

const assignmentRules = computed(() => rules.value.filter(r => r.type === 'ASSIGNMENT'));

const escalationRules = ref([
  { id: 1, name: 'Ticket SLA Breach', description: 'Escalate unresolved support tickets', isActive: true, entity: 'Ticket', trigger: 'SLA breach',
    triggerCount: 23, levels: [
  ...
  { id: 2, name: 'Deal Stale Alert', description: 'Escalate when deal has no activity for 7 days', isActive: true, entity: 'Deal', trigger: 'No activity > 7 days',
    triggerCount: 15, levels: [
  ...
  { id: 3, name: 'Lead Response Time', description: 'Escalate if lead not contacted within 1 hour', isActive: true, entity: 'Lead', trigger: 'No contact > 1 hour',
    triggerCount: 45, levels: [
...
const roundRobinPools = ref([
  { id: 1, name: 'Sales Team Pool', entity: 'Leads', method: 'Round Robin', isActive: true, totalAssigned: 234, nextAssignee: 'Sara M.', maxLoad: 20,
    members: [
  ...
  { id: 2, name: 'Support Agents', entity: 'Tickets', method: 'Least Loaded', isActive: true, totalAssigned: 187, nextAssignee: 'Khalid I.', maxLoad: 15,
    members: [
...
const saveRule = () => {
  if (!newRule.value.name) { ElMessage.warning('Rule name required'); return; }
  rules.value.push({
    id: Date.now(), name: newRule.value.name, description: '',
    isActive: true, type: newRule.value.type, matchType: newRule.value.matchType,
    conditions: newRule.value.conditions.map(c => ({ field: c.field, operator: c.operator, value: c.value })),
    assignTo: newRule.value.assignTo, assignMethod: newRule.value.method, triggerCount: 0
  });
