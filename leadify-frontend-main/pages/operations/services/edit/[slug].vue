<template lang="pug">
OperationsServiceForm( :loading="loading" @submit="submitForm" :data="service")
  .flex.items-center.justify-between.mb-8
    .title.font-bold.text-2xl.mb-1.capitalize {{ $t('operations.services.edit') }}
...
const { t } = useI18n();
useHead({
  title: computed(() => `App HP Tech | ${t('operations.services.edit')}`)
});
definePageMeta({
...
async function submitForm(values: Service) {
  loading.value = true;
  await updateService({ ...values, id: route.params.slug as string });
  loading.value = false;
}
</script>
