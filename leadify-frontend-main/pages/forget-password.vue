<template>
  <div class="login-page forgot-password relative w-full h-screen overflow-hidden flex items-center justify-center" :style="{ backgroundColor: 'var(--bg-primary)' }">
    ...
    <div class="relative z-10 w-full max-w-[1200px] px-4 flex flex-col md:flex-row items-center justify-between gap-12 lg:gap-20">
      ...
      <div class="hidden md:flex flex-1 flex-col items-start space-y-8 animate-fade-in-left">
        <img :src="themeStore.isLight ? '/images/dark-logo.png' : '/images/light-logo.png'" alt="Leadify CRM" class="h-16 w-auto drop-shadow-2xl" />
        <div class="space-y-4">
          ...
          <div class="relative z-10">
            ...
            <div v-else class="text-center py-6 animate-fade-in">
              ...
              <p :style="{ color: 'var(--text-muted)' }" class="mb-10 leading-relaxed">
                {{ $t('auth.resetLinkSent') || "We've sent a password reset link to" }} <br>
              ...
              >
                {{ $t('auth.didntReceive') || "Didn't receive email?" }} <span class="text-primary font-medium">{{ $t('auth.tryAgain') || 'Try again' }}</span>
              </button>
            </div>
          </div>
        </div>
