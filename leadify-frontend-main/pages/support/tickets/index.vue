...
div
  ...
  .flex.items-center.justify-between.mb-8
    .title.font-bold.text-2xl.mb-1.capitalize {{ $t('support.supportTickets') }}
    .flex.items-center.gap-x-3
      ExportButton(
        :data="exportData"
        :columns="exportColumns"
        filename="support-tickets-export"
        :title="$t('support.ticketsReport')"
      )
      NuxtLink(to="/support/tickets/create")
        el-button(size="large" type="primary" :icon="Plus" class="!rounded-2xl") {{ $t('support.newTicket') }}
  ...
  .glass-card.py-8.animate-entrance
    .px-6.mb-6
      el-tabs(v-model="activeTab" @tab-change="handleTabChange")
        ...
        el-tab-pane(:label="$t('tickets.open')" name="OPEN")
        el-tab-pane(:label="$t('tickets.inProgress')" name="IN_PROGRESS")
        el-tab-pane(:label="$t('tickets.waitingCustomer')" name="WAITING_CUSTOMER")
        el-tab-pane(:label="$t('tickets.resolved')" name="RESOLVED")
        el-tab-pane(:label="$t('tickets.closed')" name="CLOSED")

    //- Filters
    .px-6.flex.items-center.flex-wrap.gap-3.mb-6
      .input.table-search(class="w-full md:w-[250px]")
        el-input(size="large" style="height:50px" v-model="search" :placeholder="$t('support.searchTickets')" clearable @input="debounceLoad")
          template(#prefix)
            el-icon
              Search
      el-select(v-model="priorityFilter" clearable :placeholder="$t('support.priority')" @change="loadTickets" class="w-40" size="large")
        el-option(v-for="p in priorityOptions" :key="p.value" :value="p.value" :label="p.label")
      el-switch(v-model="assignedToMe" :active-text="$t('support.assignedToMe')" @change="loadTickets" class="ml-2")
    ...
    .px-6
      el-table(:data="tickets" v-loading="loading" style="width: 100%" @row-click="handleRowClick" row-class-name="cursor-pointer" stripe)
        ...
        el-table-column(:label="$t('support.subject')" min-width="280")
          template(#default="{ row }")
            .flex.flex-col
        ...
        el-table-column(:label="$t('support.assignee')" width="160")
          template(#default="{ row }")
            span(v-if="row.assignee") {{ row.assignee.name }}
            span.text-gray-400(v-else) {{ $t('support.unassigned') }}
        el-table-column(:label="$t('customerSuccess.client')" width="160")
          template(#default="{ row }")
            span(v-if="row.client") {{ row.client.clientName }}
            span.text-gray-400(v-else) --
        el-table-column(:label="$t('support.created')" width="140")
          template(#default="{ row }")
