  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-12 gap-4">
      ...
      <div class="col-span-12 md:col-span-4 glass-panel p-6 rounded-2xl text-center">
        <h3 class="text-sm font-medium text-slate-400 mb-2">Net Promoter Score</h3>
      ...
      <div class="col-span-12 md:col-span-4 glass-panel p-6 rounded-2xl">
        <h3 class="text-sm font-medium text-slate-400 mb-4">Response Breakdown</h3>
        <div class="space-y-3">
          <div>
        ...
        <div class="mt-4 text-xs text-slate-500 text-center">{{ npsData.totalResponses }} total responses</div>
      </div>
      ...
      <div class="col-span-12 md:col-span-4 grid grid-cols-2 gap-3">
        <div class="glass-panel p-4 rounded-xl text-center">
          ...
          <div class="text-xs text-slate-500 mt-1">Total Surveys</div>
        </div>
        <div class="glass-panel p-4 rounded-xl text-center">
        ...
        <div class="glass-panel p-4 rounded-xl text-center">
          ...
          <div class="text-xs text-slate-500 mt-1">Total Responses</div>
        </div>
        <div class="glass-panel p-4 rounded-xl text-center">
          <div class="text-2xl font-bold text-amber-400">{{ avgResponseRate }}%</div>
          <div class="text-xs text-slate-500 mt-1">Avg Response Rate</div>
        </div>
      </div>
    </div>
      ...
      <div v-for="survey in surveys" :key="survey.id" class="glass-panel p-5 rounded-xl hover:border-primary-500/30 transition-all">
        <div class="flex justify-between items-start mb-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-slate-500">{{ survey.type }} - {{ survey.questionCount }} questions</p>
            </div>
          </div>
        ...
        <div class="grid grid-cols-4 gap-3 my-3 py-3 border-t border-b border-slate-800/60 text-center">
          <div>
            ...
            <div class="text-[10px] text-slate-500">Sent</div>
          </div>
          <div>
            <div class="text-sm font-bold text-blue-400">{{ survey.responses }}</div>
            <div class="text-[10px] text-slate-500">Responses</div>
          </div>
          <div>
    ...
    <!-- Recent Feedback -->
    <div class="glass-panel p-6 rounded-2xl">
      <h3 class="text-sm font-medium text-slate-300 mb-4">Recent Feedback</h3>
      <div class="space-y-3">
    ...
    <el-dialog v-model="showCreateDialog" title="Create Survey" width="560px">
      <el-form label-position="top">
        ...
        <el-form-item label="Schedule">
          <el-date-picker v-model="newSurvey.startDate" type="date" placeholder="Start date" class="w-full" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="showCreateDialog = false">Cancel</el-button>
        <el-button type="primary" @click="createSurvey">Create Survey</el-button>
      </template>
    </el-dialog>
  </div>
</template>
