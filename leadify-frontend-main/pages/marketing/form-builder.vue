  ...
  <div class="space-y-6">
    ...
    <el-tabs v-model="activeTab" class="glass-tabs">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="form in forms" :key="form.id" class="glass-panel p-5 rounded-xl hover:border-primary-500/30 transition-all group">
            ...
            <div class="grid grid-cols-3 gap-2 my-3 py-3 border-t border-b border-slate-800/60">
              ...
              <div class="text-center">
                <div class="text-sm font-bold" :class="form.conversionRate > 20 ? 'text-emerald-400' : 'text-amber-400'">{{ form.conversionRate }}%</div>
            ...
            <div class="flex gap-2">
              ...
              <el-button size="small" text @click="previewForm(form)">
                <Icon name="ph:eye" class="w-4 h-4 mr-1" /> Preview
              </el-button>
              <el-button size="small" text @click="copyFormLink(form)">
                <Icon name="ph:link" class="w-4 h-4 mr-1" /> Link
              </el-button>
              <el-button size="small" text @click="viewSubmissions(form)">
                <Icon name="ph:list-bullets" class="w-4 h-4 mr-1" /> Data
              </el-button>
            </div>
          </div>
        </div>
      ...
      <el-tab-pane label="Templates" name="templates">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div v-for="tmpl in templates" :key="tmpl.id" class="glass-panel p-5 rounded-xl hover:border-primary-500/30 transition-all cursor-pointer" @click="useTemplate(tmpl)">
      ...
      <el-tab-pane label="Builder" name="builder">
        <div class="grid grid-cols-12 gap-4">
          ...
          <!-- Form Canvas -->
          <div class="col-span-6 glass-panel p-6 rounded-xl min-h-[400px]"
            @dragover.prevent
            @drop="onDropField"
          >
            ...
            <div v-for="(field, idx) in currentForm.fields" :key="idx" class="mb-4 p-3 rounded-lg border border-slate-800/60 hover:border-primary-500/30 transition group relative">
          ...
          <div class="col-span-3 glass-panel p-4 rounded-xl">
            ...
            <el-button type="primary" class="w-full mt-4" @click="saveForm">
              <Icon name="ph:floppy-disk-bold" class="w-4 h-4 mr-2" /> Save Form
            </el-button>
          </div>
        </div>
      </el-tab-pane>
...
interface FormField { type: string; label: string; placeholder: string; required: boolean; }
interface FormDef {
  title: string; description: string; submitText: string; redirectUrl: string;
  notifyOnSubmit: boolean; mapToEntity: string; fields: FormField[];
}

const currentForm = ref<FormDef>({
  title: 'Contact Us', description: 'Get in touch with our team', submitText: 'Submit',
  redirectUrl: '', notifyOnSubmit: true, mapToEntity: 'lead', fields: []
});

const forms = ref([
  { id: 1, name: 'Website Contact Form', type: 'Lead Capture', isActive: true, submissions: 342, views: 1820, conversionRate: 18.8, fieldCount: 5, updatedAt: '2026-02-18' },
  { id: 2, name: 'Event Registration', type: 'Registration', isActive: true, submissions: 128, views: 450, conversionRate: 28.4, fieldCount: 8, updatedAt: '2026-02-15' },
  { id: 3, name: 'Product Feedback Survey', type: 'Survey', isActive: false, submissions: 89, views: 210, conversionRate: 42.4, fieldCount: 12, updatedAt: '2026-02-10' },
  { id: 4, name: 'Newsletter Signup', type: 'Lead Capture', isActive: true, submissions: 567, views: 3200, conversionRate: 17.7, fieldCount: 3, updatedAt: '2026-02-19' },
  { id: 5, name: 'Demo Request', type: 'Lead Capture', isActive: true, submissions: 45, views: 320, conversionRate: 14.1, fieldCount: 7, updatedAt: '2026-02-17' },
  { id: 6, name: 'Partner Application', type: 'Application', isActive: false, submissions: 23, views: 150, conversionRate: 15.3, fieldCount: 15, updatedAt: '2026-01-20' }
]);

const templates = ref([
  { id: 1, name: 'Contact Us', icon: 'ph:envelope-simple-bold', gradient: 'from-blue-500 to-blue-600', fieldCount: 5, description: 'Standard contact form with name, email, phone, subject, message.', tags: ['Lead Capture', 'Basic'] },
  { id: 2, name: 'Event Registration', icon: 'ph:ticket-bold', gradient: 'from-purple-500 to-purple-600', fieldCount: 8, description: 'Event signup with attendee details, dietary preferences, session selection.', tags: ['Events', 'Registration'] },
  { id: 3, name: 'Customer Feedback', icon: 'ph:star-bold', gradient: 'from-amber-500 to-amber-600', fieldCount: 10, description: 'NPS-style feedback form with rating scales and open-ended questions.', tags: ['Survey', 'Feedback'] },
  { id: 4, name: 'Job Application', icon: 'ph:briefcase-bold', gradient: 'from-emerald-500 to-emerald-600', fieldCount: 12, description: 'Job application form with resume upload, experience, education.', tags: ['HR', 'Application'] },
  { id: 5, name: 'Product Demo Request', icon: 'ph:presentation-chart-bold', gradient: 'from-rose-500 to-rose-600', fieldCount: 7, description: 'Demo request with company info, use case, and preferred time.', tags: ['Sales', 'Demo'] },
  { id: 6, name: 'Newsletter Signup', icon: 'ph:newspaper-bold', gradient: 'from-teal-500 to-teal-600', fieldCount: 3, description: 'Simple email capture with optional name and interests.', tags: ['Marketing', 'Simple'] }
]);
...
const getFormTypeBg = (type: string) => {
  const m: Record<string, string> = { 'Lead Capture': 'bg-blue-500/10', 'Registration': 'bg-purple-500/10', 'Survey': 'bg-amber-500/10', 'Application': 'bg-emerald-500/10' };
  return m[type] || 'bg-slate-500/10';
};
const getFormTypeIcon = (type: string) => {
  const m: Record<string, string> = { 'Lead Capture': 'ph:magnet-bold', 'Registration': 'ph:ticket-bold', 'Survey': 'ph:chart-bar-bold', 'Application': 'ph:clipboard-text-bold' };
  return m[type] || 'ph:file-bold';
};
const getFormTypeColor = (type: string) => {
  const m: Record<string, string> = { 'Lead Capture': 'text-blue-400', 'Registration': 'text-purple-400', 'Survey': 'text-amber-400', 'Application': 'text-emerald-400' };
  return m[type] || 'text-slate-400';
};

const addFieldToForm = (type: string) => {
  const labels: Record<string, string> = {
    text: 'Text Field', email: 'Email Address', phone: 'Phone Number', textarea: 'Message',
    dropdown: 'Select Option', checkbox: 'Agreement', date: 'Date', rating: 'Rating', file: 'File Upload'
  };
  currentForm.value.fields.push({
    type, label: labels[type] || 'New Field', placeholder: `Enter ${labels[type]?.toLowerCase() || 'value'}`, required: false
  });
  if (activeTab.value !== 'builder') activeTab.value = 'builder';
};

const onDropField = () => {
  if (dragField) { addFieldToForm(dragField); dragField = ''; }
};
...
const editForm = (form: any) => { activeTab.value = 'builder'; ElMessage.info(`Editing: ${form.name}`); };
const previewForm = (form: any) => ElMessage.info(`Preview: ${form.name}`);
const copyFormLink = (form: any) => { navigator.clipboard?.writeText(`https://forms.example.com/${form.id}`); ElMessage.success('Link copied!'); };
const viewSubmissions = (form: any) => ElMessage.info(`${form.submissions} submissions for ${form.name}`);
const useTemplate = (tmpl: any) => { ElMessage.info(`Using template: ${tmpl.name}`); activeTab.value = 'builder'; };
const saveForm = () => { ElMessage.success('Form saved!'); activeTab.value = 'forms'; };
</script>
