      ...
      el-tab-pane(:label="$t('recruitment.applicants')" name="applicants")
        ...
        .flex.items-center.gap-3.mb-4.flex-wrap
          el-select(v-model="applicantStageFilter" :placeholder="$t('recruitment.allStages')" clearable size="large" style="width: 180px")
            el-option(:label="$t('recruitment.allStages')" value="")
          ...
          el-select(v-model="applicantJobFilter" :placeholder="$t('recruitment.allJobs')" clearable size="large" style="width: 220px" filterable)
            el-option(:label="$t('recruitment.allJobs')" value="")
      ...
      .grid.gap-4(class="grid-cols-1 md:grid-cols-2")
        el-form-item(:label="$t('recruitment.type')")
          el-select(v-model="postingForm.type" class="w-full")
            el-option(:label="$t('recruitment.fullTime')" value="FULL_TIME")
            el-option(:label="$t('recruitment.partTime')" value="PART_TIME")
      ...
      .grid.gap-4(class="grid-cols-1 md:grid-cols-2")
        el-form-item(:label="$t('recruitment.source')")
          el-select(v-model="applicantForm.source" class="w-full" filterable allow-create :placeholder="$t('recruitment.selectSource')")
            el-option(:label="$t('recruitment.linkedin')" value="LinkedIn")
            el-option(:label="$t('recruitment.indeed')" value="Indeed")
            el-option(:label="$t('recruitment.referral')" value="Referral")
            el-option(:label="$t('recruitment.website')" value="Website")
            el-option(:label="$t('recruitment.jobBoard')" value="Job Board")
            el-option(:label="$t('recruitment.other')" value="Other")
...
const handlePostingSelectionChange = (rows: Record<string, unknown>[]) => {
  selectedPostings.value = rows;
};
const handleApplicantSelectionChange = (rows: Record<string, unknown>[]) => {
...
function navigateToPosting(row: unknown) {
...
// ─── Posting CRUD ────────────────────────────────────────
function openPostingDialog(item?: unknown) {
  if (item?.id) {
...
async function handleDeletePosting(row: unknown) {
  try {
...
// ─── Applicant CRUD ──────────────────────────────────────
function openApplicantDialog(item?: unknown) {
  if (item?.id) {
