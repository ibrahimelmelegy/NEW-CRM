  ...
  .flex.items-center.justify-between.mb-8
    div
      .flex.items-center.gap-2.mb-2
        NuxtLink.text-sm.font-medium(to="/hr/training" style="color: var(--accent-color, #7849ff)") {{ $t('hr.training.title') || 'Training & LMS' }}
        Icon(name="ph:caret-right" size="14" style="color: var(--text-muted)")
        span.text-sm(style="color: var(--text-muted)") {{ program?.title || '...' }}
      h1.text-2xl.font-bold(style="color: var(--text-primary)") {{ program?.title || ($t('hr.training.programDetail') || 'Program Detail') }}
      p.text-sm.mt-1(v-if="program" style="color: var(--text-muted)")
        | {{ program.type }} | {{ program.category || '--' }} | {{ program.instructor || ($t('hr.training.noInstructor') || 'No instructor') }}
    .flex.items-center.gap-3
      el-button(size="large" type="primary" @click="openEnrollDialog()" class="!rounded-2xl")
        Icon(name="ph:user-plus-bold" size="16")
        span.ml-1 {{ $t('hr.training.enrollParticipant') || 'Enroll Participant' }}
  ...
  template(v-else-if="program")
    ...
    .glass-card.p-6.rounded-2xl.mb-6
      .grid.gap-6(class="grid-cols-1 md:grid-cols-2 lg:grid-cols-5")
        div
          p.text-xs.uppercase.tracking-wider.mb-1(style="color: var(--text-muted)") {{ $t('hr.training.status') || 'Status' }}
          el-tag(:type="getStatusType(program.status)" effect="dark" size="default") {{ program.status }}
        div
          p.text-xs.uppercase.tracking-wider.mb-1(style="color: var(--text-muted)") {{ $t('hr.training.type') || 'Type' }}
          el-tag(effect="plain" size="default") {{ program.type }}
        div
          p.text-xs.uppercase.tracking-wider.mb-1(style="color: var(--text-muted)") {{ $t('hr.training.duration') || 'Duration' }}
          p.text-lg.font-bold(style="color: var(--text-primary)") {{ program.durationHours || 0 }}h
        div
          p.text-xs.uppercase.tracking-wider.mb-1(style="color: var(--text-muted)") {{ $t('hr.training.dateRange') || 'Date Range' }}
          p.text-sm.font-semibold(style="color: var(--text-primary)") {{ formatDate(program.startDate) }} - {{ formatDate(program.endDate) }}
        div
          p.text-xs.uppercase.tracking-wider.mb-1(style="color: var(--text-muted)") {{ $t('hr.training.capacity') || 'Capacity' }}
      ...
      div.mt-5(v-if="program.description")
        p.text-xs.uppercase.tracking-wider.mb-2(style="color: var(--text-muted)") {{ $t('hr.training.description') || 'Description' }}
      ...
      el-col(:xs="24" :md="12")
        .glass-card.p-6.rounded-2xl.h-full
          .flex.items-center.gap-2.mb-4
            ...
            h3.text-base.font-bold(style="color: var(--text-primary)") {{ $t('hr.training.completionStats') || 'Completion Statistics' }}

          .grid.gap-4(class="grid-cols-2 lg:grid-cols-4")
            .text-center
              .text-2xl.font-bold(style="color: #3b82f6") {{ statusCounts.enrolled }}
              .text-xs.mt-1(style="color: var(--text-muted)") {{ $t('hr.training.enrolled') || 'Enrolled' }}
            .text-center
              .text-2xl.font-bold(style="color: #f59e0b") {{ statusCounts.inProgress }}
              .text-xs.mt-1(style="color: var(--text-muted)") {{ $t('hr.training.inProgress') || 'In Progress' }}
            .text-center
              .text-2xl.font-bold(style="color: #22c55e") {{ statusCounts.completed }}
              .text-xs.mt-1(style="color: var(--text-muted)") {{ $t('hr.training.completed') || 'Completed' }}
            .text-center
              .text-2xl.font-bold(style="color: #ef4444") {{ statusCounts.dropped }}
              .text-xs.mt-1(style="color: var(--text-muted)") {{ $t('hr.training.dropped') || 'Dropped' }}

          //- Completion rate bar
          .mt-4
            .flex.items-center.justify-between.mb-2
              span.text-sm.font-medium(style="color: var(--text-primary)") {{ $t('hr.training.completionRate') || 'Completion Rate' }}
        ...
        .glass-card.p-6.rounded-2xl.h-full
          .flex.items-center.justify-between.mb-4
            .flex.items-center.gap-2
              ...
              h3.text-base.font-bold(style="color: var(--text-primary)") {{ $t('hr.training.materials') || 'Training Materials' }}
          ...
          .text-center.py-6(v-else)
            ...
            p.text-sm.mt-2(style="color: var(--text-muted)") {{ $t('hr.training.noMaterials') || 'No materials uploaded yet' }}
    ...
    .glass-card.p-6.rounded-2xl
      .flex.items-center.justify-between.mb-4
        .flex.items-center.gap-2
          ...
          h3.text-base.font-bold(style="color: var(--text-primary)") {{ $t('hr.training.participants') || 'Participants' }} ({{ enrollments.length }})
        .flex.items-center.gap-2
          el-select(v-model="participantStatusFilter" clearable :placeholder="$t('common.status') || 'All'" size="default" style="width: 140px")
      ...
      el-table(:data="filteredParticipants" v-loading="loading" style="width: 100%" stripe)
        ...
        el-table-column(:label="$t('hr.training.enrolledDate') || 'Enrolled'" width="140")
          template(#default="{ row }")
            span.text-sm(style="color: var(--text-muted)") {{ formatDate(row.createdAt) }}
        el-table-column(:label="$t('hr.training.progress') || 'Progress'" width="200" align="center")
          template(#default="{ row }")
            .flex.items-center.gap-2
        ...
        el-table-column(:label="$t('hr.training.rating') || 'Rating'" width="120" align="center")
          template(#default="{ row }")
        ...
        el-table-column(:label="$t('common.actions') || 'Actions'" width="140" align="center")
          template(#default="{ row }")
            .flex.items-center.justify-center.gap-1
              el-button(v-if="row.status !== 'COMPLETED' && row.status !== 'DROPPED'" text type="success" size="small" @click="handleComplete(row)" :title="$t('hr.training.markComplete') || 'Complete'")
      ...
      .text-center.py-8(v-if="!enrollments.length && !loading")
        ...
        p.text-sm.mt-2(style="color: var(--text-muted)") {{ $t('hr.training.noParticipants') || 'No participants enrolled yet' }}

  //- 404 / Not Found
  .text-center.py-20(v-else-if="!loading")
    Icon(name="ph:warning-circle" size="48" style="color: var(--text-muted)")
    p.text-lg.font-semibold.mt-4(style="color: var(--text-primary)") {{ $t('common.notFound') || 'Program not found' }}
    NuxtLink(to="/hr/training")
      el-button.mt-4(type="primary") {{ $t('common.goBack') || 'Go Back' }}

  //- Enroll Dialog
  el-dialog(v-model="enrollDialogVisible" :title="$t('hr.training.enrollParticipant') || 'Enroll Participant'" width="480px" destroy-on-close)
    el-form(label-position="top")
      el-form-item(:label="$t('hr.training.employee') || 'Employee'" required)
        el-select(v-model="enrollForm.employeeId" filterable :placeholder="$t('hr.training.selectEmployee') || 'Select employee'" class="w-full")
          el-option(v-for="emp in employees" :key="emp.id" :label="emp.name" :value="emp.id")
    template(#footer)
      el-button(@click="enrollDialogVisible = false") {{ $t('common.cancel') || 'Cancel' }}
      el-button(type="primary" @click="handleEnroll" :loading="saving") {{ $t('hr.training.enroll') || 'Enroll' }}

  //- Edit Enrollment Dialog
  el-dialog(v-model="editEnrollmentVisible" :title="$t('hr.training.editEnrollment') || 'Edit Enrollment'" width="500px" destroy-on-close)
    el-form(label-position="top" v-if="editingEnrollment")
      ...
      el-form-item(:label="$t('hr.training.feedback') || 'Feedback'")
        el-input(v-model="editEnrollForm.feedback" type="textarea" :rows="3" :placeholder="$t('hr.training.feedbackPlaceholder') || 'Enter participant feedback...'")
      el-form-item(:label="$t('hr.training.rating') || 'Rating'")
        el-rate(v-model="editEnrollForm.rating" :max="5" show-text :texts="['Poor', 'Below Average', 'Average', 'Good', 'Excellent']")
    template(#footer)
      el-button(@click="editEnrollmentVisible = false") {{ $t('common.cancel') || 'Cancel' }}
      el-button(type="primary" @click="handleUpdateEnrollment" :loading="saving") {{ $t('common.save') || 'Save' }}

  //- Edit Program Dialog
  el-dialog(v-model="editProgramVisible" :title="$t('hr.training.editProgram') || 'Edit Program'" width="600px" destroy-on-close)
    el-form(:model="programForm" label-position="top")
      ...
      .grid.gap-4(class="grid-cols-2")
        el-form-item(:label="$t('hr.training.type') || 'Type'")
    ...
    template(#footer)
      el-button(@click="editProgramVisible = false") {{ $t('common.cancel') || 'Cancel' }}
      el-button(type="primary" @click="handleUpdateProgram" :loading="saving") {{ $t('common.save') || 'Save' }}

  //- Add Material Dialog
  el-dialog(v-model="showAddMaterial" :title="$t('hr.training.addMaterial') || 'Add Material'" width="480px" destroy-on-close)
    el-form(label-position="top")
      el-form-item(:label="$t('hr.training.materialName') || 'Name'" required)
        el-input(v-model="materialForm.name" :placeholder="$t('hr.training.materialNamePlaceholder') || 'e.g., Course Handbook'")
      el-form-item(:label="$t('hr.training.materialUrl') || 'URL'" required)
        el-input(v-model="materialForm.url" :placeholder="$t('hr.training.materialUrlPlaceholder') || 'https://...'")
      el-form-item(:label="$t('hr.training.materialType') || 'Type'")
    ...
    template(#footer)
      el-button(@click="showAddMaterial = false") {{ $t('common.cancel') || 'Cancel' }}
      el-button(type="primary" @click="handleAddMaterial" :loading="saving") {{ $t('common.add') || 'Add' }}
</template>
...
async function loadProgram() {
  ...
  try {
    ...
    ElNotification({ type: 'error', title: t('common.error') || 'Error', message: t('common.loadFailed') || 'Failed to load program' });
  } finally {
    loading.value = false;
  }
}
...
async function handleEnroll() {
  if (!enrollForm.employeeId) {
    ElNotification({ type: 'warning', title: t('common.warning') || 'Warning', message: t('common.fillRequired') || 'Please select an employee' });
    return;
  }
  saving.value = true;
  try {
    ...
    ElNotification({ type: 'success', title: t('common.success') || 'Success', message: t('hr.training.enrolled') || 'Participant enrolled' });
    await loadProgram();
  } catch (err: any) {
    ElNotification({ type: 'error', title: t('common.error') || 'Error', message: err?.message || t('common.error') || 'Failed to enroll' });
  } finally {
    saving.value = false;
  }
}
...
async function handleUpdateEnrollment() {
  ...
  try {
    ...
    ElNotification({ type: 'error', title: t('common.error') || 'Error', message: t('common.error') || 'Failed to update' });
  } finally {
    saving.value = false;
  }
}
...
async function handleComplete(row: any) {
  try {
    await ElMessageBox.confirm(
      t('hr.training.confirmComplete') || 'Mark this enrollment as completed?',
      t('common.confirm') || 'Confirm',
      { type: 'info' }
    );
    await useApiFetch(`hr/training/enrollments/${row.id}/complete`, 'PUT');
    ElNotification({ type: 'success', title: t('common.success') || 'Success', message: t('hr.training.completedSuccess') || 'Marked as completed' });
    await loadProgram();
  } catch { /* cancelled */ }
}
...
async function handleDeleteEnrollment(row: any) {
  try {
    await ElMessageBox.confirm(
      t('common.confirmDelete') || 'Are you sure you want to remove this enrollment?',
...
async function handleAddMaterial() {
  ...
  try {
    ...
    ElNotification({ type: 'success', title: t('common.success') || 'Success', message: t('hr.training.materialAdded') || 'Material added' });
    await loadProgram();
  } catch {
    ElNotification({ type: 'error', title: t('common.error') || 'Error', message: t('common.error') || 'Failed to add material' });
  } finally {
    saving.value = false;
  }
}
