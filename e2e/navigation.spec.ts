 ...
 *   - [data-testid="nav-sales-leads"]      → NuxtLink to /sales/leads
 *   - [data-testid="nav-sales-deals"]      → NuxtLink to /sales/deals
 *   - [data-testid="nav-sales-clients"]    → NuxtLink to /sales/clients
 *   - [data-testid="nav-settings-*"]       → Settings sub-items
...
test.describe('Main Navigation', () => {
  ...
  test.describe('Sidebar Navigation', () => {
    ...
    test('should display sidebar with main menu items', async ({ page }) => {
      ...
      // Use data-testid="sidebar" for reliable sidebar targeting
      const sidebar = page.locator('[data-testid="sidebar"]');
      const hasSidebar = await sidebar.isVisible({ timeout: 10000 }).catch(() => false);

      if (!hasSidebar) {
        // Fallback to class-based selector if testid not yet rendered
        const fallbackSidebar = page.locator('[class*="sidebar"], nav, aside').first();
    ...
    test('should expand Sales submenu to show Leads, Deals, Clients', async ({ page }) => {
      ...
      // Try data-testid nav link first (set by Menu.vue dynamically)
      const leadsNavLink = page.locator('[data-testid="nav-sales-leads"]');
      let hasLeads = await leadsNavLink.isVisible({ timeout: 5000 }).catch(() => false);
      ...
      if (!hasLeads) {
        ...
        if (await salesMenu.isVisible({ timeout: 5000 }).catch(() => false)) {
          ...
          hasLeads = await leadsNavLink.isVisible({ timeout: 5000 }).catch(() => false);
        }
  ...
  test.describe('Sidebar via Click Navigation', () => {
    ...
    test('should navigate to Leads page via sidebar click', async ({ page }) => {
      ...
      // Use data-testid nav link generated by Menu.vue (nav-sales-leads)
      const leadsNavLink = page.locator('[data-testid="nav-sales-leads"]');
      if (await leadsNavLink.isVisible({ timeout: 5000 }).catch(() => false)) {
        await leadsNavLink.click();
        await page.waitForTimeout(3000);
        await expect(page).toHaveURL(/leads/);
      } else {
        // Fallback: expand Sales submenu then click Leads
    ...
    test('should navigate to Deals page via sidebar click', async ({ page }) => {
      const authenticated = await navigateAuthenticated(page, '/');
      if (!authenticated) { test.skip(); return; }

      await dismissOverlay(page);

      // Use data-testid nav link generated by Menu.vue (nav-sales-deals)
      const dealsNavLink = page.locator('[data-testid="nav-sales-deals"]');
      if (await dealsNavLink.isVisible({ timeout: 5000 }).catch(() => false)) {
        await dealsNavLink.click();
        await page.waitForTimeout(3000);
        await expect(page).toHaveURL(/deals/);
      } else {
        // Fallback: expand Sales submenu then click Deals
        const salesMenu = page.locator('text=/Sales/i').first();
        if (await salesMenu.isVisible({ timeout: 5000 }).catch(() => false)) {
          await salesMenu.click();
          await page.waitForTimeout(500);
        ...
        const dealsLink = page.locator('a:has-text("Deals"), [href*="/sales/deals"]').first();
