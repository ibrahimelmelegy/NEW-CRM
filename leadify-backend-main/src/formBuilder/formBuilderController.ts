...
class FormBuilderController {
  async createTemplate(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      wrapResult(res, await service.createTemplate(req.body, req.user!.tenantId!), 201);
    } catch (e) {
      next(e);
    }
  }
  async getTemplates(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      wrapResult(res, await service.getTemplates(req.query, req.user!.tenantId!));
    } catch (e) {
      next(e);
    }
  }
  async updateTemplate(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      wrapResult(res, await service.updateTemplate(Number(req.params.id), req.body));
    } catch (e) {
      next(e);
    }
  }
  ...
  async submitForm(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const result = await service.submitForm(Number(req.params.id), formData, {
  ...
  async trackFormView(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      wrapResult(res, await service.trackFormView(Number(req.params.id)));
    } catch (e) {
      next(e);
    }
  }
  async getFormByToken(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!form) {
        wrapResult(res, { message: 'Form not found' }, 404);
        return;
      }
      wrapResult(res, form);
    } catch (e) {
  ...
  async getSubmissions(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      wrapResult(res, await service.getSubmissions(req.query, req.user!.tenantId!));
    } catch (e) {
      next(e);
    }
  }
  async getFormAnalytics(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      wrapResult(res, {
        ...
        conversionRate:
          result.totalSubmissions > 0 ? Math.round((result.totalSubmissions / Math.max(result.totalSubmissions, 1)) * 100 * 10) / 10 : 0,
