...
class TrainingController {
  ...
  async enrollWithCapacity(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const tenantId = req.user!.tenantId!;
      const result = await trainingService.enrollWithCapacityCheck(Number(req.body.programId), Number(req.body.employeeId), tenantId);
      wrapResult(res, result, 201);
    } catch (e) {
      next(e);
    }
  ...
  async checkPrerequisites(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const result = await trainingService.checkPrerequisites(Number(req.params.programId), Number(req.query.employeeId));
      wrapResult(res, result);
    } catch (e) {
      next(e);
    }
