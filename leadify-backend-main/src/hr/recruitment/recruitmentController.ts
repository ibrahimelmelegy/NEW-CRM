...
class RecruitmentController {
  ...
  async moveApplicantStage(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const result = await recruitmentService.moveApplicantStage(Number(req.params.id), req.body.stage);
      wrapResult(res, result);
    } catch (e) {
      next(e);
    }
  ...
  async checkDuplicate(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const existing = await recruitmentService.checkDuplicateApplicant(req.query.email as string, Number(req.query.jobPostingId));
