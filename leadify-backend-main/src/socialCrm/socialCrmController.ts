...
class SocialCrmController {
  ...
  async createPost(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      wrapResult(res, await service.createPost(req.body, req.user!.tenantId!), 201);
    } catch (e) {
      next(e);
    }
  }
  async getPosts(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      wrapResult(res, await service.getPosts(req.query, req.user!.tenantId!));
    } catch (e) {
      next(e);
    }
  }
  async updatePost(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      wrapResult(res, await service.updatePost(Number(req.params.id), req.body));
    } catch (e) {
      next(e);
    }
  }
