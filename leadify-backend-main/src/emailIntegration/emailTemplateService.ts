...
class EmailTemplateService {
  ...
  async createTemplate(data: Record<string, unknown>, tenantId?: string): Promise<ComposerTemplate> {
    return ComposerTemplate.create({ ...data, tenantId });
  }

  async updateTemplate(id: string, data: Record<string, unknown>): Promise<ComposerTemplate> {
    const template = await ComposerTemplate.findByPk(id);
    if (!template) throw new BaseError(ERRORS.NOT_FOUND);
