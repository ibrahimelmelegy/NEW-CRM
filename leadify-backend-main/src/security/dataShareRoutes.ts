...
import { getAllSharingRules, setDefaultAccess, shareRecord, revokeShare, getRecordShares } from './dataShareService';
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const rule = await setDefaultAccess(entityType, defaultAccess, hierarchyAccess !== undefined ? hierarchyAccess : true, description);
      wrapResult(res, rule);
    } catch (error) {
      next(error);
    }
 ...
 *         description: Invalid input
 */
router.post('/share', authenticateUser, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.user) {
 ...
 */
router.delete('/share/:id', authenticateUser, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.get('/shares/:entityType/:entityId', authenticateUser, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const entityType = req.params.entityType as string;
