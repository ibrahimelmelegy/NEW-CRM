...
class AIChatService {
    ...
    async askCRM(question: string, userId: number, tenantId?: string): Promise<ChatResponse> {
        ...
        try {
            ...
            return {
                answer: 'I encountered an issue processing your question. Please try rephrasing it, or ask about specific metrics like deal counts, pipeline value, or lead status.',
            };
        }
    }
    ...
    private analyzeQuestionFallback(question: string): any {
        ...
        return { intent: 'general', module: 'none', answer: 'I can help with questions about your leads, deals, clients, invoices, and opportunities. Try asking "How many deals closed this month?" or "What is the total pipeline value?"' };
    }

    private extractTimeRange(q: string): any | undefined {
        const now = new Date();
        ...
        switch (plan.intent) {
            ...
            case 'recent': {
                ...
                const order = plan.sort
                    ? [[plan.sort.field, plan.sort.order]]
                    : [['createdAt', 'DESC']];
        ...
        try {
            ...
            const completion = await client.chat.completions.create({
                messages: [
                    {
                        role: 'system',
                        content: 'You are a helpful CRM assistant. Given the user\'s question and query results, provide a concise, clear natural language answer. Use numbers, bullet points for lists, and be specific. Do not mention JSON or technical details. Keep responses under 200 words.',
                    },
                    {
                        role: 'user',
        ...
        switch (plan.intent) {
            ...
            case 'top':
                if (data.items?.length > 0) {
                    const names = data.items.slice(0, 5).map((item: any) =>
            ...
            case 'status':
                if (data.breakdown?.length > 0) {
                    const lines = data.breakdown.map((item: any) => {
