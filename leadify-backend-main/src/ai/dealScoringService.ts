...
class DealScoringService {
        ...
        try {
            ...
            const completion = await client.chat.completions.create({
                messages: [
                    {
                        role: 'system',
                        content: 'You are a sales coach. Given deal data and risk factors, provide 1-2 specific, actionable suggestions to improve the deal outcome. Be concise - each suggestion should be one sentence. Return as JSON array of strings.',
                    },
                    {
                        role: 'user',
    ...
    private async getAvgDealAge(): Promise<number> {
        ...
        const now = new Date();
        const totalAge = deals.reduce(
            (sum, d) => sum + Math.floor((now.getTime() - new Date(d.createdAt).getTime()) / (1000 * 60 * 60 * 24)),
            0
        );
