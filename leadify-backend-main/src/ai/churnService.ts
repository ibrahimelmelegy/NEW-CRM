...
class ChurnService {
    ...
    async getChurnDashboard(user: User): Promise<ChurnDashboardData> {
        ...
        const mapLeads = (leads: Lead[], riskLevel: 'HIGH' | 'MEDIUM' | 'LOW'): ChurnLead[] => {
            return leads.map(lead => {
                const daysSince = lead.lastContactDate
                    ? Math.floor((now.getTime() - new Date(lead.lastContactDate).getTime()) / (1000 * 60 * 60 * 24))
                    : 999;
        ...
        const allAtRisk = [
            ...mapLeads(highRiskLeads, 'HIGH'),
            ...mapLeads(mediumRiskLeads, 'MEDIUM'),
            ...mapLeads(lowRiskLeads, 'LOW')
