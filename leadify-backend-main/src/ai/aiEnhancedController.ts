    ...
    async getDailyInsights(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (totalLeadsThisWeek > 0 || newLeadsPrevWeek > 0) {
                const conversionRate = totalLeadsThisWeek > 0
                    ? Math.round((convertedLeadsThisWeek / totalLeadsThisWeek) * 100)
                    : 0;
                const leadChange = newLeadsPrevWeek > 0
                    ? Math.round(((newLeadsThisWeek - newLeadsPrevWeek) / newLeadsPrevWeek) * 100)
