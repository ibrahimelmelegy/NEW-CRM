...
export const aiInsightsController = {
  async getDailyInsights(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      for (const deal of stalledDeals as Record<string, unknown>[]) {
        ...
        const daysSince = lastActivity
          ...
          : Math.floor((now.getTime() - new Date(deal.createdAt as string).getTime()) / (1000 * 60 * 60 * 24));
      ...
      if (stalledDealsList.length > 0) {
        const totalValue = stalledDealsList.reduce((sum: number, d: unknown) => sum + (Number((d as Record<string, unknown>).price) || 0), 0);
        insights.push({
          id: 'stalled-deals',
          type: 'warning',
      ...
      if (overdueInvoices.length > 0) {
        const totalOverdue = (overdueInvoices as unknown[]).reduce((sum: number, inv: unknown) => sum + (Number((inv as Record<string, unknown>).amount) || 0), 0);
        insights.push({
          id: 'overdue-invoices',
          type: 'danger',
      ...
      const pipelineTotal = Number((pipelineValue as Record<string, unknown>).total) || 0;
      if (pipelineTotal > 0) {
        insights.push({
          id: 'pipeline-value',
          type: 'info',
